...
export async function getProducts(request, limit, offset){
    const {data: {products, header, maxSize, attributeTypes}} = await axios.post(`http://localhost:8000/api/products?limit=${limit}&offset=${offset}`, {
    ...
    return {products, columns, maxSize, attributeTypes};
}
